        ...
        <div class="announcement-popup-content">
            @if($activePopup->image_path)
                <div class="announcement-popup-image">
                    <img src="{{ $activePopup->image_url }}" alt="{{ $activePopup->title }}">
                </div>
            @endif
            
            @if($activePopup->text_content)
                <div class="announcement-popup-text">
                    <p>{{ $activePopup->text_content }}</p>
                </div>
            @endif
            
            @if($activePopup->link_url)
                <div class="announcement-popup-action">
                    <a href="{{ $activePopup->link_url }}" class="btn btn-primary announcement-popup-btn">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const delay = {{ $activePopup->delay_seconds ?? 3 }} * 1000;
    const showOnce = {{ $activePopup->show_once_per_session ? 'true' : 'false' }};
    const popupId = 'announcement_popup_{{ $activePopup->id }}';
