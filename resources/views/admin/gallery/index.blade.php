        ...
        <div class="col-md-12">
            <div class="card shadow-sm">
                <div class="card-header d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #4A90E2 0%, #357ABD 100%); color: white;">
                    ...
                    <span class="badge bg-white text-primary">{{ $images->count() }}/10 Images</span>
                </div>
                <div class="card-body">
                            ...
                            @if($images->count() > 0)
                                <div id="sortableGallery" class="row">
                                    @foreach($images as $image)
                                        <div class="col-md-6 col-lg-4 mb-4 gallery-item" data-id="{{ $image->id }}" style="cursor: move;">
                                            <div class="card h-100 shadow-sm" style="transition: all 0.3s;">
                                                <img src="{{ $image->image_url }}" class="card-img-top" alt="{{ $image->title ?? 'Gallery Image' }}" style="height: 200px; object-fit: cover;">
                                                <div class="card-body">
                                                    <h6 class="card-title">{{ $image->title ?? 'Untitled' }}</h6>
                                                    <p class="card-text text-muted small">{{ $image->description ? Str::limit($image->description, 80) : 'No description' }}</p>
                                                    
                                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                                        <small class="text-muted">Order: {{ $image->order }}</small>
                                                        <div class="form-check form-switch">
                                                            <input class="form-check-input status-toggle" type="checkbox" data-id="{{ $image->id }}" {{ $image->is_active ? 'checked' : '' }}>
                                                            <label class="form-check-label small">Active</label>
                                                        </div>
                                                    </div>
                                                    
                                                    <div class="d-flex gap-2">
                                                        <button class="btn btn-sm btn-outline-primary flex-fill" onclick="editImage({{ $image->id }}, '{{ $image->title }}', '{{ $image->description }}')">
                                                            <i class="fas fa-edit"></i> Edit
                                                        </button>
                                                        <form action="{{ route('admin.gallery.destroy', $image->id) }}" method="POST" class="flex-fill" onsubmit="return confirm('Are you sure you want to delete this image?');">
                                                            @csrf
                                                            @method('DELETE')
...
document.getElementById('homepageToggle').addEventListener('change', function() {
    ...
    fetch('{{ route("admin.gallery.toggle-homepage") }}', {
        method: 'POST',
        headers: {
